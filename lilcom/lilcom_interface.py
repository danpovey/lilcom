...
def compress(input, axis=-1, lpc_order=5, default_exponent=0, out=None):
    ...
    if not isinstance(input, np.ndarray):
        raise TypeError("Expected input to be of type numpy.ndarray, got {}"
                        .format(type(input)))
    if not (input.dtype in [np.int16, np.float32, np.float64]):
        raise TypeError("Expected data-type of NumPy array to be int16, float32 or float64 "
                        "and it to be nonempty, got dtype={}, size={}"
                        .format(input.dtype, input.size))
    if input.size == 0:
    ...
    if not (isinstance(axis, int) and axis >= -num_axes and axis < num_axes):
        raise ValueError("axis={} invalid or out of range for ndim={}"
                         .format(axis, num_axes))

    # lpc_order
    if not (isinstance(lpc_order, int) and lpc_order >= 0 and lpc_order <= 15):
        raise ValueError("lpc_order={} is not valid".format(lpc_order))
    # default_exponent
    if not (isinstance(default_exponent, int) and default_exponent >= 0 and
            default_exponent <= 15):
        raise ValueError("default_exponent={} is not valid"
                         .format(default_exponent))
    ...
    if not isinstance(out, np.ndarray):
        raise TypeError("Expected `out` to be of type numpy.ndarray, got {}"
                        .format(type(out)))
    if not (out.dtype == np.int8 and out.shape == out_shape):
        raise ValueError("Expected `out` to have dtype=int8 and shape={}, got {} and {}"
                         .format(out_shape, out.dtype, out.shape))
    ...
    if input.dtype == np.float32:
        ...
        if ret != 0:
            raise RuntimeError("Something went wrong in lilcom compression (possibly "
                               "infinities or NaNs in the input data), return code {}"
                               .format(ret))
    else:
...
def decompress(input, axis=-1, out=None, dtype=None):
    ...
    if input.shape[axis] < 5:
        raise ValueError("Input data has shape {}, dimension must be at "
                         "least 5 on axis={}.  Possibly wrong `axis` value?"
    ...
    if out is None:
        if not (dtype in [np.int16, np.float32, np.float64]):
            raise TypeError("`dtype` must be one of int16, float32, float64, got: {}"
                            .format(dtype))
    ...
    if not (out.dtype in [np.int16, np.float32, np.float64]):
        raise TypeError("dtype of output should be int16, float32 or float64, got {}"
                        .format(out))
    if out.shape != out_shape:
        raise ValueError("shape of output should be {}, got {}"
    ...
    if out.dtype == np.int16:
        ...
        if ret >= 1000:
            if ret == 1003:
                ...
                raise RuntimeError("Something went wrong in lilcom decompression, return code = {}"
                                   .format(ret))
        return out_pre_swapping_axes
    else:
