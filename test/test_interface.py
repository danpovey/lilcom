    ...
    for axis in [-1, 1, 0, -2]:
        for use_out in [False, True]:
            ...
            b = lilcom.compress(a, axis=axis,
                                out=(np.empty(out_shape, dtype=np.int8)
                                     if use_out else None))
            # decompressing as int16, float or double should give the same
            # result except it would be scaled by 1/32768
            for d in [np.int16, np.float32, np.float64]:
                c = lilcom.decompress(b,
                                      dtype=(None if use_out else d),
                                      axis=axis,
                                      out=(np.empty(a.shape, dtype=d)
                                           if use_out else None))

                a2 = a.astype(np.float32) * (1.0/32768.0
                                             if d != np.int16 else 1.0)
                c2 = c.astype(np.float32)
                rel_error = (np.fabs(a2 - c2)).sum() / (np.fabs(a2)).sum()
                print("Relative error in int16 compression(decompressing as {},"
                      "axis={}, use_out={}) is {}"
                      .format(d, axis, use_out, rel_error))
...
def test_double():
    ...
    rel_error = (np.fabs(a - c)).sum() / (np.fabs(a)).sum()
    print("Relative error in double compression, decompressing as double, is: "
          , rel_error)
