...
def test_float():
    for axis in [-1, 1, 0, -2]:
        ...
        print("Sizes are {},{}".format(a.shape,b.shape))
        c = lilcom.decompress(b, dtype=np.float32, axis=axis)
...
def test_int16():
    ...
    for axis in [-1, 1, 0, -2]:
        ...
        for d in [np.int16, np.float32, np.float64]:
            c = lilcom.decompress(b, dtype=d, axis=axis)
            ...
            print("Relative error in int16 compression (decompressing as {}, axis={}) is {}".format(
                    d, axis, rel_error))
